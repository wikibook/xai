...
|29|정정|CrossEntropy에 대한 Marginal Probability |CrossEntropy를 구할 때 공식 중 1/2이 잘못되었습니다. 클래스간 전체 엔트로피 감소를 관찰해야하므로 좌항에는 3/16이, 우항에는 13/16을 곱해야합니다. 결과값은 0.9812가 나옵니다.|김현규님|
|-|보충|데이터를 알고리즘에 학습시키기 위하여 전처리 과정 중 Normalisation이나 Standardisation으로 스케일링을 하는 데, 이후 학습된 Deep Neural Network 모델을 LIME이나 SHAP을 적용하여 설명을 추출할 때 원래 데이터로 복구하여 해석해야 사람에게 더 이해하기 쉬운 설명이 될 것으로 생각됩니다. 스케일링된 학습데이터를 원래 데이터로 복구하여 설명할 수 있는 쉬운 방법이 있나요? 물론, 스케일링 된 데이터만으로 분석하더라도 어떤 feature가 중요하고 원하는 예측결과가 나오기 위하여 특정 feature value를 증가시켜야 할 지, 감소시켜야 할 지는 파악할 수 있지만 스케일이 달라서 얼마만큼 증감을 시켜야 할 지는 알 수 가 없습니다. 아무리 구글링으로 관련 자료를 찾아봐도 스케일링과 XAI technique 관련 내용은 찾기가 어렵습니다.| 스케일링 방법론이 모델의 직관적인 해석에 방해가 된다는 말씀으로 이해됩니다. 다른 예제를 보았을 때는 이런 과정 없이 바로 데이터를 학습에 활용하다보니 해석이 용이한데, 위에서 말한 Scaling 기법들은 한 번 더 역변환을 해 주어야 하기 때문에 직관적이지 않은 해석법이 아닌지 하는 의문이신 것 같습니다. Scaling은 ‘역변환’이 가능할 때 해석력을 갖습니다. 만약 scaling 이후에 원래대로 복원되지 않는다면 스케일링은 기법은 리얼월드와 접점을 잃습니다. 저희가 예제로 자주 사용하는 scipy는 스케일링 메서드에 ‘변환->역변환’ 코드도 함께 잘 구현해두었습니다. 보통은 `.inverse_transform(X)`라는 메서드로 함께 제공됩니다. 그리고 메일에서 언급하신 Standard Scaler는 inverse_transform()을 제공합니다. 이렇게 되면 머신러닝의 해석 결과로 나온 scale을 역변환 함으로써 실제로는 어느정도의 영향을 미치는지 파악할 수 있을 것입니다. 저희가 아는 다수의 스케일러들은 역변환을 제공합니다. 몇몇 Scaling은 역변환이 불가하기도 합니다. 버케팅 기법 같이 미분 불가능한 커널을 스케일러로 사용한 경우가 될 테지요. 이 경우에는 원본 데터에 대한 데이터 구간 추론은 가능하나 데이터의 원상 복구는 불가합니다.|권민오님|
